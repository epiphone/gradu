...
Pricing among FaaS providers is more homogeneous. FaaS providers typically charge users by the combination of number of invocations and their execution durations. Execution duration is counted in 100ms increments and rounded upwards, with the unit price of 100 milliseconds of execution depending on the selected function size. Each parallel function execution is billed separately. For example in AWS Lambda the price per invocation is \$0.0000002, and the price for 100ms of execution ranges from \$0.000000208 for 128MB functions to \$0.000004897 for 3008MB functions \parencite{awslambda0218}. At this price point, running a 300ms execution on a 128MB function 10 million times would add up to about \$8.25. The other major providers operate roughly at the same price point \parencite{microsoft18azureFunctions,ibm18cloudFunctions,google18cloudFunctions}. Most providers also offer a free tier of a certain amount of free computation each month. The AWS Lambda free tier for example includes 800,000 seconds of computation on the 512MB function.
