...
It is a common occurrence for service requests to fail in distributed systems. A service could be temporarily unavailable or a slow network could lead to request timeout. In case of temporary interruptions like these, a simple retry mechanism is usually enough for the system-at-large to recover and maintain operation. There are however occasions when components are unavailable for extended periods of time so that it becomes pointless to retry operations that are bound to fail. Longer service interruptions like this can result from a variety of reasons ranging from programming errors to network or hardware failure. \parencite{microsoft18cloudPatterns}
