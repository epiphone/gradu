...
\subsection{Event Fan-out} \label{subsec:FanoutEvents}
...
% \begin{figure}[h]
%   \centering
%   \includegraphics[width=0.4\textwidth]{patterns/event-fan-out.png}
%   \caption{Event Fan-out}
%   \label{fig:eventFanOut}
% \end{figure}

\textbf{Solution:} Subscribe multiple functions to a notification service, publish notification on event occurrence.

The Event Processor pattern \ref{subsec:Eventprocessing} is applicable within cases of a 1-to-1 relationship between events and tasks, as exemplified above with image upload triggering thumbnail generation. In other cases, however, a single event can result in multiple independent tasks. For example the image upload event could as well trigger a database update and a notification email, adding up to three self-contained and parallel tasks. Most event sources however only support invoking one function per event, which leaves us with a couple of options. First, we could subscribe the image upload event to a new function that in turn asynchronously invokes any number of processor functions: a sort of a parallel Sorting Function \label{subsec:routingFunction}. This approach comes with the operational overhead of needing to set up and maintain an additional function per each event fan-out. An alternative solution is take advantage of the platform services and use a cloud-based messaging service that supports multiple subscribers per channel, such as the AWS Simple Notification Service \parencite{awslambda0218}. Now instead of subscribing a function directly to an event source, functions subscribe to a message channel that the event source publishes a message to upon event occurrence. In addition to achieving parallel fan-out to multiple functions, the pattern has the added benefit of loosed coupling between event sources and handler functions. \parencite{sbarski2017serverless}
