...
The patterns are grouped into rough categories for better readability. Also the interrelation of the patterns is sketched out in figure \ref{fig:patternLanguage}. This figure forms a \textit{pattern language}, i.e. a structural organization of pattern relations \parencite{rotem12soa}.

\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{pattern-language.png}
  \caption{Pattern language}
  \label{fig:patternLanguage}
\end{figure}

% TODO flesh out each section intro
...
The Circuit Breaker is similar to the SOA pattern of Service Watchdog \parencite{rotem12soa} in that both implement self-healing by means of restricted service access. What distincts the two is the responsible party: the Service Watchdog depends on an integrated component inside the service to monitor its state whereas the Circuit Breaker only acts on the basis of failed requests, external to the service. This distinction makes the Circuit Breaker easier to deploy against third-party services outside our control.
