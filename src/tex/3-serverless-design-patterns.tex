...
\subsection{Throttle} \label{subsec:throttle}

\textbf{Problem:} A rapidly scaling FaaS function overwhelms a downstream component such as a database.
...
\textbf{Solution:} Throttle invocations to avoid DDoSing yourself.

Serverless platforms scale out horizontally in response to increased demand. AWS Lambda for example is capable of up to a 1000 concurrent function instances \parencite{awslambda0218}. This sort of configuration-free elasticity is a major benefit and selling point of the FaaS model and plays well together with similarly autoscaling BaaS services. Scalability can also be a pitfall, however, when interacting with conventional non-scaling resources. In such hybrid architectures a rapidly scaling FaaS function has the potential to overwhelm components that lack similar scaling properties or are otherwise not designed to accept high-volume traffic \parencite{cncf18serverlessWG}. This can have the effect of rendering the whole system unavailable in an essentially self-inflicted Denial of Service-attack. For example, how would an on-premise database with strict connection limits fare with said 1000 concurrent instances? Ideally we would just stick to BaaS and cloud services in favour of non-scaling components, i.e. forego hybrid architectures altogether. Naturally this isn't always feasible as few systems have the luxury of not depending on any pre-existing components. Migrating these components into the cloud or deploying enough on-premise hardware to meet peak demand can also be prohibitively costly and organizationally difficult.

Addressing the problem then calls for some mechanism to protect downstream services from intermittent load spikes caused by concurrent FaaS instances. The simplest solution is to apply a limit on the number of concurrent function instances; a configuration option most FaaS platforms provide. AWS documentation for example states that ``concurrency controls are sometimes necessary to protect specific workloads against service failure as they may not scale as rapidly as Lambda'', particularly in case of ``sensitive backend or integrated systems that may have scaling limitations'' and ``database connection pool restrictions such as a relational database, which may impose concurrent limits'' \parencite{aws18serverlessLens}. While managing to ease the pressure on downstream services, this solution causes non-responsivity in consuming applications and arguably fails to take full advantage of the FaaS platform.

The Throttler represents a more sophisticated approach based on buffering the bursts of concurrent requests originating from FaaS and resolving them asynchronously in a more even pace. A serverless adaptation of the Queue-Based Load Leveling cloud design pattern \parencite{microsoft18cloudPatterns}, the Throttler adds a message queue between the function and the downstream service so that instead of directly requesting the service and waiting for a reply, the function pushes the request into the queue and immediately responds to the consumer with an acknowledgement of receipt. It's then the job of another function to consume the queue and resolve service requests. By adjusting queue consumption rate, request spikes can be resolved in a smoother pace to avoid service resource exhaustion and thus function timeouts. The queue also provides a further level of robustness as requests are not lost even in case of service outage. Finally, utilizing the Throttler to ``control the flow of data between two services'' in a hypothetical issue tracking system, \textcite{baldini17currentTrends} improve on the queue consumer's network efficiency by implementing batching.

Decoupled Invocation \parencite{rotem12soa}: ``Utilize the Decoupled Invocation pattern and separate replies from requests: acknowledge receipt at the service edge, put the request on a reliable queue, and then load-balance and prioritize the handler components that read from the queue.''
