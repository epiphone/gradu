...
% \textbf{Problem:} How to avoid extra costs while accessing third party cloud services?
\textbf{Problem:} How to coordinate access to third party cloud services while avoiding extra costs?

% \begin{figure}[h]
%   \centering
%   \includegraphics[width=0.6\textwidth]{patterns/thick-client.png}
...
Serverless applications, as described in chapter \ref{cha:serverless}, typically consist of a mixture of third party cloud services (BaaS) and custom logic implemented as serverless functions (FaaS). However, applying serverless functions in the manner of a traditional layered web application backend, as a gateway between the client and external resources, has a few drawbacks. First of all, using FaaS as middle layer in front of cloud resources directly translates into extra costs: on top of paying for the service call, one has to pay for the function invocation, function execution for the duration of the network call as well as the data transfer between the service and the FaaS provider. Secondly, a middle layer of FaaS results into extra network hops, thus increasing latency and reducing user experience. \parencite{sbarski2017serverless}

Let the client orchestrate workflows.

so pushing the workflow coordination to the client significantly reduces hosting costs \parencite{adzic2017serverless}

Closely related to the Valet Key pattern described in \ref{subsec:valetKey}.
