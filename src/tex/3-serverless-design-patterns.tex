...
One authorization pattern that fits the above requirements is the Valet Key. In this pattern the client, when looking to access a resource, first requests access to said resource from a special authorizer serverless function. The authorizer function first checks whether the client has access to the resource and then signs and returns an access token that is both short-lived and tightly restricted to this specific resource. Now for any subsequent calls until token expiration, the client can request the resource directly without going through an any additional server layer. At the same time the pattern avoids leaking resource credentials outside the authorizer function; token signature is enough for the resource to validate request authenticity. It is to be observed that the pattern relies heavily on cloud services' fine-grained authorization models, as the access token may need to be restricted to a single file in file storage or a single key in a key/value store, for example.
