...
A \textit{bulkhead} refers to a sectioned partition in a ship's hull designed to prevent the whole ship from filling with water and sinking in case of an accident. Similarly in the cloud computing context the Bulkhead pattern refers to isolating ``elements of an application into pools so that if one fails, the others will continue to function'' \parencite{microsoft18cloudPatterns}. The idea is to prevent a situation where a single service, when facing resource exhaustion due to a load peak or some sort of an application error, brings down other services with it causing a cascading failure and large-scale service interruption. The way this is achieved is by partitioning service resources in self-contained blocks divided along lines of expected load and availability requirements. Now even with one service out of the XXX the system can still deliver some level of functionality.

What are the serverless implications of this inter-service resource contention problem? It seems that FaaS platforms already ensure resource isolation as each function instance runs in its own resource-constrained container. Declaring the problem solved might be premature, however, namely for two reasons: first of all as shown by XXX, function instances, albeit isolated on a container-level, do suffer from resource contention in the VM level; a byproduct of some platforms' scheduling and allocation strategy of placing a customer's functions in the same VM. Secondly, a single function can contain multiple execution paths with varying latencies, which can lead to intra-service resource contention. XXX for example demonstrate a user service function with endpoints for both fetching a token and creating an altogether new one: a case of an infrequently called high-latency operation degrading the performance of a frequently called low-latency one.
While the first problem is only addressable by platform providers, the second problem is mitigated by a finer function granularity, i.e. separating high-latency workflows into their own functions.

In serverless computing the Bulkhead pattern then comes to mean splitting ...

smaller, more isolated components easier to replace; high load on A doesn't effect B, i.e. performance isolation; serverless does away with economic incentives for bundling anyway

Operations overhead; partition is not trivial; FaaS platforms cannot guarantee functions are deployed on separate VMs
