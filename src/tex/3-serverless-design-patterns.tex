...
\textbf{Problem:} Rapid scalability of FaaS function causes a downstream service to fail under load.
...
Serverless functions scale out horizontally in response to increased demand. AWS Lambda for example is capable of up to a 1000 concurrent instances \parencite{awslambda0218}. This sort of configuration-free elasticity is a major benefit and selling point of the whole FaaS model and plays well together with similarly autoscaling BaaS services. These properties can also be a pitfall, however, when interacting with conventional non-scaling resources. In such hybrid architectures a rapidly scaling FaaS function can quickly overwhelm a more rigid component and render the whole system unavailable in the process. For example how would a conventional on-premise database with rigid connection limits fare with said 1000 concurrent instances?

``Will my serverless platform overwhelm any other components such as a database? Can I manage or throttle back-pressure?'' \parencite{cncf18serverlessWG}

Three options of either sticking to BaaS, limiting FaaS concurrency, and Throttler
The problem leaves us with three alternative solutions. The first one is to just stick to elastic cloud and BaaS services instead of integrating with any non-scalable resources in the first place; that is, foregoing hybrid architectures altogether.

Lambda concurrency limit \textcite{aws18serverlessLens}.
...
Decoupled Invocation \parencite{rotem12soa}
