...
Overall the two Image Manager implementations perform quite differently performance-wise, as one might expect. The serverful application's response times stay in the range of a few seconds until service resources are exhausted and rise sharply after that along with the error rate. The serverless application on the other hand adapts to increasing load with constant execution durations, presumably until the platform's concurrency limit is reached. While server response times and function execution durations are not directly comparable (the latter lacks the original image upload and event propagation latencies) they do manage to get across the drastic difference in elasticity.

The serverless Image Manager's elasticity, while mostly beneficial, points towards the necessity of protecting any non-scaling downstream services with something like the Throttler pattern in order not to overwhelm them. The results also demonstrate the efficiency of the Bulkhead pattern's granular deployment units, as the LQIP rendering operation can scale independently without allocating redundant resources elsewhere to the system.

As for the problem of cold starts, an initial delay in image processing was deemed acceptable and didn't thus necessitate implementing the Function Warmer pattern. The pattern can however be appropriate (TODO necessary?) in case of synchronous invocations such as the authorizer function where any delay leaks to the consumer, potentially freezing the user interface. Deriving from that (TODO ??), another workaround over the cold start problem is to design workflows in an eventually consistent manner so that they don't rely on synchronous or blocking invocations to begin with.
