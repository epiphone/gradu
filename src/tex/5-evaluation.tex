...
Omitting costs for Cloud Storage, Cloud Vision API and ReCaptcha since they are identical. Data transfer inside Google Cloud is mostly free.

VM costs \$18.3 per month and reaches up to 3 requests per second. This means 7776000 images per month which is about 2225GB of data assuming a 300KB image.

Omitting the authorization step, processing a single image means 3 function invocations and on average a total of 2 seconds of execution time, which with the 1024MB memory allocation adds up to \$0.0000342. Hence processing 3 images per second for a month costs \$265.9392.
...
A single load balancer forwarding rule would cost \$0.025/hour = \$18/month. 2225GB of ingress traffic is charged at \$17.8 by the load balancer. Hence a setup that achieves minimal redundancy with two VMs behind a load balancer costs \$72.4.
